...
export async function GET(request: NextRequest) {
  try {
    ...
    const positionMetrics = ['QB', 'RB', 'WR', 'TE'].map((position) => {
      ...
      const avgR2 =
        positionData.reduce((sum: number, stat: any) => sum + (stat.r2 || 0), 0) /
        positionData.length
      const avgRMSE =
        positionData.reduce((sum: number, stat: any) => sum + (stat.std_prediction_error || 0), 0) /
        positionData.length
      const totalRecords = positionData.reduce(
    ...
    // Overall statistics
    const totalRecords = aggregatedStats.reduce(
      (sum: number, stat: any) => sum + (stat.player_count || 0),
      0
    )
