...
export async function GET(request: NextRequest) {
  try {
    ...
    const sleeperPlayerIds = searchParams.get("player_ids")?.split(",").map(id => id.trim()) || []
    ...
    const { data: stats, error } = await supabaseServer
      ...
      .select("player_gsis_id, player_display_name, fantasy_ppg, total_fantasy_points, games_played")
