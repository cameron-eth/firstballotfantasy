  ...
  const connectUser = useCallback(
    async (usernameOverride?: string) => {
      ...
      try {
        ...
        setError(
          err.message || 'Failed to connect to Sleeper. Please check your username and try again.'
        )
        setLoading(false)
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-300 text-sm">Choose a league to view drafts</p>
                    </div>
            ...
            {user && selectedLeagueId && drafts.length > 0 && !selectedDraft && (
              <Card className="bg-slate-800 border-slate-700">
                <CardHeader>
                  ...
                  <p className="text-gray-300 text-sm">Choose a draft to view the board</p>
                </CardHeader>
                <CardContent>
                    ...
                    <SelectContent className="bg-slate-800 border-slate-700 text-slate-100">
                      {drafts.map((draft) => (
                        <SelectItem
                          ...
                          className="bg-slate-800 text-slate-100 hover:bg-slate-700 focus:bg-slate-700 focus:text-yellow-400"
                        >
                          <div className="flex items-center space-x-2">
                            <div
                              className={`w-2 h-2 rounded-full ${getDraftStatusColor(draft.status)}`}
                            ></div>
                            <span>
                              {draft.type} Draft ({draft.settings.rounds} rounds,{' '}
