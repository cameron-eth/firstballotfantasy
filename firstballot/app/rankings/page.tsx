...
import { Search, ArrowUpDown, Gem, Crown, Star, Award, Trophy, Medal, Shield } from 'lucide-react'
...
export default function RankingsPage() {
  ...
  const getRowBgColor = (player: PlayerRanking): string => {
    if (isDiamondTier(player)) {
      return 'bg-cyan-950/40 border-l-4 border-l-cyan-400/60 shadow-[inset_0_0_20px_rgba(34,211,238,0.1)]'
    }

    const tierNum = extractTierNumber(player.tier)
    const colors: Record<number, string> = {
      1: 'bg-yellow-900/30 border-l-4 border-l-yellow-500/60 shadow-[inset_0_0_20px_rgba(250,204,21,0.15)]', // Gold background for Elite with glow
      2: 'bg-purple-950/20', // Purple background for Tier 2
      3: 'bg-blue-950/20', // Blue background for Tier 3
      4: 'bg-green-950/20', // Green background for Tier 4
      5: 'bg-orange-950/20', // Orange background for Tier 5
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <main className="w-full px-4 py-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white font-mono mb-2">DYNASTY RANKINGS</h1>
                    ...
                    <tbody>
                      {paginatedRankings.map((player) => {
                        ...
                        return (
                          <tr
                            key={player.rank}
                            className={`border-b border-slate-700/50 hover:bg-slate-700/40 transition-colors ${rowBg}`}
                          >
                            ...
                            <td className="py-3 px-4 text-gray-300">{player.age}</td>
