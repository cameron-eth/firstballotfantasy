  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {starters.map((playerId, index) => {
        ...
        let playerPoints = 0
        if (startersPoints && startersPoints[index] !== undefined && startersPoints[index] !== null) {
          playerPoints = startersPoints[index]
        } else if (playersPoints && playersPoints[playerId] !== undefined && playersPoints[playerId] !== null) {
        ...
        return (
          <div key={`${playerId}-${index}`} className="group relative">
            ...
            <Card className={`relative border transition-all duration-200 ${
              gamePlayed 
                    ...
                    {gamePlayed && (
                      <div className="mt-2 space-y-1">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="text-gray-500 font-mono">{projectedPoints.toFixed(1)} pts</span>
                        </div>
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-gray-400 font-semibold">Actual:</span>
                          <div className="flex items-center space-x-1.5">
                            <span className={`font-mono font-bold ${
                              overperformed ? 'text-green-400' : underperformed ? 'text-red-400' : 'text-slate-300'
                            }`}>
                            ...
                            {overperformed && (
                              <div className="flex items-center space-x-0.5 text-green-400">
                                ...
                                <span className="text-[10px] font-bold">+{pointsDiff.toFixed(1)}</span>
                              </div>
                            )}
                            {underperformed && (
                    ...
                    {!gamePlayed && (
                      <div className="mt-2">
                        <div className="flex items-center justify-between text-xs mb-1">
                          ...
                          <span className="text-yellow-400 font-mono font-bold">{projectedPoints.toFixed(1)} pts</span>
                        </div>
                        <div className="flex justify-end">
                          <Badge 
                            variant="outline" 
