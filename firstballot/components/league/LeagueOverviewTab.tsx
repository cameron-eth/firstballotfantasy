            ...
            <div className="hidden md:flex items-center space-x-2 sm:space-x-3">
              <button
                ...
                onClick={() => {
                  ...
                  window.location.href = currentLeagueId
                    ? `/trade-market?leagueId=${currentLeagueId}`
                    : '/trade-market'
                }}
              >
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {teams
                ...
                .map((team, index) => (
                  <div key={team.rosterId} className="group relative">
                ...
                transactions.map((tx) => {
                  ...
                  return (
                    <div key={tx.transactionId} className="group relative">
                      ...
                      <div className="relative bg-slate-700/30 backdrop-blur-sm border border-slate-600/50 hover:border-yellow-400/50 transition-all duration-200 p-4 sm:p-5 rounded-lg min-w-[280px] sm:min-w-[320px] max-w-sm flex-shrink-0">
                        <div className="flex items-center justify-between mb-2">
                          <Badge
                            variant="outline"
                            className="text-xs px-2 py-1 bg-yellow-400/20 text-yellow-400 border-yellow-400/30 font-mono font-bold"
                          >
                            {tx.type.toUpperCase()}
                          </Badge>
                          ...
                          <div className="text-gray-300 space-y-1">
                            {addedPlayers.length > 0 && (
                              <div className="text-xs">
                                ...
                                <span>{addedPlayers.join(', ')}</span>
                              </div>
                            )}
                            {droppedPlayers.length > 0 && (
                              <div className="text-xs">
                                <span className="text-red-400 font-semibold">Dropped:</span>{' '}
                                <span>{droppedPlayers.join(', ')}</span>
                              </div>
                            )}
                          </div>
                        )}
                          ...
                          <div className="space-y-1">
                            {addedPlayers.length > 0 && (
                              <div className="text-xs">
                                ...
                                <span className="text-gray-300">{addedPlayers.join(', ')}</span>
                              </div>
                            )}
                            {droppedPlayers.length > 0 && (
                              <div className="text-xs">
                                <span className="text-red-400 font-semibold">Dropped:</span>{' '}
                                <span className="text-gray-300">{droppedPlayers.join(', ')}</span>
                              </div>
                            )}
                          </div>
                        )}
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-400 mt-2 pt-2 border-t border-slate-600/50">
                          ...
                          <Badge
                            variant="outline"
                            className="text-xs px-1 py-0 bg-green-400/20 text-green-400 border-green-400/30 font-mono"
                          >
                            {tx.status}
                          </Badge>
                        </div>
