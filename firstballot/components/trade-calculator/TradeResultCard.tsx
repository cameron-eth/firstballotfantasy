...
export function TradeResultCard({ result }: TradeResultCardProps) {
  ...
  const renderAssetRow = (item: TradeResult['side1'][number], color: 'blue' | 'purple') => (
    <div
      key={`${color}-${item.name}-${item.position ?? 'asset'}-${item.rank ?? 'na'}-${item.total_score}`}
      className="flex items-center justify-between p-2 md:p-4 bg-slate-900/40 rounded-lg md:rounded-xl border border-white/5"
    >
      <div className="flex items-center gap-2 md:gap-4">
        <div className="relative">
          ...
          {item.rank && (
            <div className={`absolute -top-1 -right-1 w-3.5 h-3.5 md:w-4 md:h-4 rounded ${color === 'blue' ? 'bg-blue-500' : 'bg-purple-500'} text-[7px] md:text-[8px] font-black font-mono flex items-center justify-center text-slate-900`}>
              {item.rank}
            </div>
          )}
        </div>
  ...
  return (
    <div className="space-y-4 md:space-y-10">
      {/* ── Mobile: single flat list, no cards ── */}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {result.side1_rank && (
                <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/20 font-mono text-[8px] px-1.5 py-0">
                  #{result.side1_rank}
                </Badge>
              )}
            </div>
          ...
          <div className="space-y-1.5">
            {result.side1.map((item) => renderAssetRow(item, 'blue'))}
            {result.side1_not_found.map((name) => (
              <div key={`missing-m-s1-${name}`} className="p-2 bg-red-500/5 border border-red-500/10 rounded-lg flex items-center justify-between">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {result.side2_rank && (
                <Badge variant="outline" className="bg-purple-500/10 text-purple-400 border-purple-500/20 font-mono text-[8px] px-1.5 py-0">
                  #{result.side2_rank}
                </Badge>
              )}
            </div>
          ...
          <div className="space-y-1.5">
            {result.side2.map((item) => renderAssetRow(item, 'purple'))}
            {result.side2_not_found.map((name) => (
              <div key={`missing-m-s2-${name}`} className="p-2 bg-red-500/5 border border-red-500/10 rounded-lg flex items-center justify-between">
                <span className="text-[9px] text-red-400 font-mono italic">{name}</span>
                <Minus className="h-3 w-3 text-red-900" />
              </div>
            ))}
      ...
      {/* ── Desktop: dossier cards side-by-side ── */}
      <div className="hidden md:grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Side 1 Detailed Breakdown */}
        <div className="relative group">
          <div className="absolute -top-2.5 left-6 px-3 py-1 bg-slate-900 border border-white/5 rounded text-[10px] font-black font-mono text-blue-400 uppercase tracking-widest z-10">
            Side 1
          </div>
          <Card className="bg-slate-950/40 backdrop-blur-sm border border-blue-500/20 rounded-2xl shadow-none overflow-hidden">
            <CardContent className="p-0">
              <div className="bg-blue-500/5 p-6 border-b border-white/5 flex justify-between items-end">
                <div>
                  <div className="text-[9px] text-slate-500 font-mono uppercase tracking-widest mb-0.5">Total</div>
                  <div className="text-4xl font-black text-white font-mono tracking-tighter">{result.side1_total.toLocaleString()}</div>
                </div>
                {result.side1_rank && (
                  <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/20 font-mono text-[9px] px-1.5">#{result.side1_rank}</Badge>
                )}
              </div>
              <div className="p-6 space-y-3">
                {result.side1.map((item) => renderAssetRow(item, 'blue'))}
                {result.side1_not_found.map((name) => (
                  <div key={`missing-side1-${name}`} className="p-2 bg-red-500/5 border border-red-500/10 rounded-lg flex items-center justify-between">
                    <span className="text-[9px] text-red-400 font-mono italic">{name}</span>
                    <Minus className="h-3 w-3 text-red-900" />
                  </div>
                ))}
        ...
        <div className="relative group">
          <div className="absolute -top-2.5 left-6 px-3 py-1 bg-slate-900 border border-white/5 rounded text-[10px] font-black font-mono text-purple-400 uppercase tracking-widest z-10">
            Side 2
          </div>
          <Card className="bg-slate-950/40 backdrop-blur-sm border border-purple-500/20 rounded-2xl shadow-none overflow-hidden">
            <CardContent className="p-0">
              <div className="bg-purple-500/5 p-6 border-b border-white/5 flex justify-between items-end">
                <div>
                  <div className="text-[9px] text-slate-500 font-mono uppercase tracking-widest mb-0.5">Total</div>
                  <div className="text-4xl font-black text-white font-mono tracking-tighter">{result.side2_total.toLocaleString()}</div>
                </div>
                {result.side2_rank && (
                  <Badge variant="outline" className="bg-purple-500/10 text-purple-400 border-purple-500/20 font-mono text-[9px] px-1.5">#{result.side2_rank}</Badge>
                )}
              </div>
              <div className="p-6 space-y-3">
                {result.side2.map((item) => renderAssetRow(item, 'purple'))}
                {result.side2_not_found.map((name) => (
                  <div key={`missing-side2-${name}`} className="p-2 bg-red-500/5 border border-red-500/10 rounded-lg flex items-center justify-between">
                    <span className="text-[9px] text-red-400 font-mono italic">{name}</span>
                    <Minus className="h-3 w-3 text-red-900" />
                  </div>
                ))}
