...
export async function cachedFetch<T>(
  ...
  const options =
    tags.length > 0 ? { ...cacheOptions, next: { ...cacheOptions.next, tags } } : cacheOptions
...
export const sleeperApi = {
  ...
  getLeagueInfo: (leagueId: string) =>
    cachedFetch(`https://api.sleeper.app/v1/league/${leagueId}`, 'LEAGUE_INFO', [
      CACHE_TAGS.LEAGUE,
    ]),

  getLeagueRosters: (leagueId: string) =>
    cachedFetch(`https://api.sleeper.app/v1/league/${leagueId}/rosters`, 'ROSTERS', [
      CACHE_TAGS.LEAGUE,
    ]),

  getLeagueUsers: (leagueId: string) =>
    cachedFetch(`https://api.sleeper.app/v1/league/${leagueId}/users`, 'USERS', [
      CACHE_TAGS.LEAGUE,
    ]),

  getLeagueMatchups: (leagueId: string, week: number) =>
    cachedFetch(`https://api.sleeper.app/v1/league/${leagueId}/matchups/${week}`, 'MATCHUPS', [
      CACHE_TAGS.LEAGUE,
    ]),
  ...
  getTradedPicks: (leagueId: string) =>
    cachedFetch(`https://api.sleeper.app/v1/league/${leagueId}/traded_picks`, 'LEAGUE_INFO', [
      CACHE_TAGS.LEAGUE,
    ]),
  ...
  getUserLeagues: (userId: string) =>
    cachedFetch(`https://api.sleeper.app/v1/user/${userId}/leagues/nfl/2025`, 'USER_SETTINGS', [
      CACHE_TAGS.USER,
    ]),

  // Draft data
  getDraft: (draftId: string) =>
    cachedFetch(`https://api.sleeper.app/v1/draft/${draftId}`, 'DRAFT_ANALYSIS', [
      CACHE_TAGS.DRAFT,
    ]),

  getDraftPicks: (draftId: string) =>
    cachedFetch(`https://api.sleeper.app/v1/draft/${draftId}/picks`, 'DRAFT_ANALYSIS', [
      CACHE_TAGS.DRAFT,
    ]),

  getDraftTradedPicks: (draftId: string) =>
    cachedFetch(`https://api.sleeper.app/v1/draft/${draftId}/traded_picks`, 'DRAFT_ANALYSIS', [
      CACHE_TAGS.DRAFT,
    ]),
}
