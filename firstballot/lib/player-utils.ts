...
export function getEspnId(player: any, espnPlayersData: any[]): string | undefined {
  ...
  let matches = activePlayers.filter((p) => p.fullName?.toLowerCase() === playerName?.toLowerCase())
  ...
  if (matches.length === 0) {
    matches = activePlayers.filter((p) => {
      ...
      return (
        espnName && searchName && (espnName.includes(searchName) || searchName.includes(espnName))
      )
    })
  }
...
export function debugEspnIdResolution(player: any, espnPlayersData: any[]): any {
  ...
  // Try exact match
  const exactMatches = activePlayers.filter(
    (p) => p.fullName?.toLowerCase() === playerName?.toLowerCase()
  )
